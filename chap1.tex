...
By the calculations above it is understandable why this estimate on $u$ instead of \eqref{eq:resolventEstimateL2} is sometimes called resolvent estimate.
...
\begin{lem}[Lem 5.2.13, \cite{tolksdorf}]
  ...
  \end{align*}
  the spaces $\Ell^p_\sigma(\Omega)$ and $(\Ell^{p'}_\sigma(\Omega))^*$ are isomorphic, where $(\Ell^{p'}_\sigma(\Omega))^*$ denotes the antidual space and $p' = p/ (p - 1)$  is the Hölger conjugate exponent of $p$. 
...
\begin{defn}
  Let $\Omega \subset \R^d$, $d \geq 2$, be a bounded Lipschitz domain and let $\varepsilon > 0$ be as in Theorem \ref{thm:stokesOperatorLp}.
  Let 
  \begin{align*}
