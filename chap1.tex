...
The first step will be to introduce nontangential approach regions.
Unaffortunately in the literature there exist at least two different concepts.
...
\begin{defn}[Nontangential approach region, Shen]
  \label{defn:nontangentialApproachRegion}
   ...
   \begin{align*}
     \ShenCone(q) \coloneqq \big\{ x \in \Omega \setminus \partial\Omega \colon |x - q| < \alpha \operatorname{dist}(x, \partial\Omega) \big\}
   \end{align*}
   If $\alpha$ si chosen sufficiently large (see Shen \cite{shen2017}) we call $\{ \ShenCone(q) \colon q \in \partial\Omega\}$ a \emph{family of nontangential approach regions}.
\end{defn}

  Note that in Shen cones $\ShenCone(p)$, we have that for $q, y \in \partial\Omega$ and $x \in \ShenCone(q)$
\begin{align}
  ...
  &\leq (\alpha + 1) |x - y|
\end{align}
where $\alpha$ is the constant from \ref{defn:nontangentialApproachRegion}. It is reasonable to choose $\alpha$ in a way that also condition (ii) from Theorem \ref{thm:smoothApproximation} holds.
...
\begin{defn}
For a function $u$ in $\Omega$ and a fixed family of nontangential approach regions, we define the nontangential maximal function $(u)^*$ by
\begin{align}
  \label{eq:defnNontangMaxFunction}
  (u)^*(q) = \sup\big\{ |u(x)| \colon x \in \ShenCone(q)\big\}
\end{align}
for $q \in \partial\Omega$.
\end{defn}

Note that Tolksdorf \cite{tolksdorf} and Shen \cite{shen2017} show that the choice of $\alpha$ for the nontangential maximal function does not affect their $p$-norms in an unpredictable way. In fact their $p$-norms for different $\alpha_1$ and $\alpha_2$ stay comparable with a constant only depending on $d$, $\alpha_1$, $\alpha_2$ and the Lipschitz character.

The above mentioned constructions of cones are not limited to cones that lay in the interior of the domain $\Omega$.
In fact the same construction can be carried out for the exterior domain $\R^d \setminus \overline{\Omega}$ yielding cones that lay outside of $\Omega$.

\begin{defn}[Nontangential convergence]
  Let $\Omega$ be a bounded Lipschitz domain and $\{\ShenCone(q) \colon q \in \partial\Omega\}$ be a family of nontangential approach regions which lies either inside or outside $\Omega$.
  Let furthermore $u$ be a function on $\Omega$ and $f$ a function on $\partial\Omega$.
  We say that $u = f$ \emph{in the sense of nontangential convergence} if 
  \begin{align*}
...
For functions $u$ on $\Omega$ we will therefore often use the notation $u_+$ to denote the \emph{inner} nontangential limit for a (implicitly) fixed family of nontangential approach regions and $u_-$ for the respective \emph{outer} nontangential limit.
