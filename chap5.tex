...
\begin{proof}
  ...
  \begin{align*}
      &\int_{\partial\Omega} | \nabla_x \{ \Gamma(p - y; \lambda) - \Gamma(p - y; 0) \} | \d\sigma(y)\\[0.5em]
  ...
  \begin{align*}
      \int_{\partial\Omega} |\nabla_x \{\Gamma(p - y; \lambda) - \Gamma(p - y; 0) \} | \d\sigma(y) \leq C \, |\lambda|^{1/2}
  \end{align*}
  ...
  The third step follows from Theorem \ref{thm:rellichExterior} considering that $\big\|\nabla_{\mathrm{tan}} u \big\|^{}_\partial \leq C\, \|\nabla u \|_{\partial}$.
  Now, extending estimate \eqref{eq:estimatef} by \eqref{eq:estimatephiplus} gives
  \begin{align*}
      \| f\|^{}_\partial 
      \leq C \, \big\| ( - (1/2) I + \K_\lambda) f \big\|^{}_\partial + C\, \Big\| \Big(\frac{\partial u}{\partial \nu} \Big)_- \Big\|^{}_\partial 
    \leq C \, \big\| \big( - (1/2) I + \K_\lambda) f\big\|_{\partial},
  \end{align*}
...
With the following lemma that looks like a reverse trace theorem, we will later show the uniqueness of solutions to the $\Ell^2$ Dirichlet problem to the Stokes resolvent system.
...
\begin{proof}
  ...
  In order to derive estimate \eqref{eq:w2estimate}, we need the Rellich-type identity
%  \begin{align}
...
\end{alignedat}
  ...
  \end{align*}
    and thanks to \eqref{eq:w2lambdaEstimate}, this expression may be bounded by
  \begin{align*}
    C_\varepsilon \int_\Omega |u|^2 \d x + \varepsilon \int_\Omega |\psi|^2 \d x.
  \end{align*}
...
\begin{proof}
  ...
  The following part of this proof will show that $u$ coincides with the solution $w \coloneqq \dlp_\lambda(g)$, $g \in \Ell^2(\partial\Omega; \C^d),$  of the $\Ell^2$ Dirichlet problem with boundary data $f$ as given by Theorem \ref{thm:exAndUniqueSolution}. 
...
\begin{rem}
  ...
  Let $t \in (1,2)$. 
  By Theorem \ref{thm:exAndUniqueSolution}, we know that a solution to the Stokes resolvent system on $\BB(0,\, t) \subsetneq \BB(0,\,2)$ with boundary values $g_t \coloneqq \operatorname{Tr}_{\partial\BB(0,\,t)} (u) \in \Ell^2(\partial\BB(0,\,t); \C^d)$ is given by a boundary layer potential $\dlp_\lambda(f_t)$.
