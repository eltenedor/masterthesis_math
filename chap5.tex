...
\end{alignedat}
  ...
  \end{align}
  where $C > 0$ depends on $d$, $\theta$ and the Lipschitz character of $\Omega$.
...
\begin{proof}
  ...
  \begin{align}
    ...
    \bigg( \int_\Omega |u|^{p_d} \d x \bigg)^{1/{p_d}} \leq C\, \bigg( \int_{\partial\Omega} |(u)^*|^2 \d \sigma \bigg)^{1/2},
  \end{align}
  where $C>0$ only depends on $d$ and the Lipschitz constant of $\Omega$.
...
\begin{rem}
  ...
  \end{align*}
  Note that for this argument to work it is crucial that $C > 0$ does only depend on $d$ and the Lipschitz character of $\Omega$.
