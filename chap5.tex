...
\begin{rem}
  Let $(u,\phi)$ be a solution of the Stokes resolvent system in the domain $\BB(x_0, r) \subset \R^d$, $d \geq 3$.
  \begin{align}
  ...
  Let $t \in (1,2)$. By Theorem \ref{thm:exAndUniqueSolution} we know that a solution to the Stokes resolvent sytsem on $\BB(0, t) \subsetneq \BB(0,2)$ with boundary values $g_t \coloneqq \operatorname{Tr}_{\partial\BB(0,t)} u \in \Ell^2(\partial\BB(0,t))$ is given by a boundary layer potential $\dlp_\lambda(f)$.
  We can use this fact to derive the desired estimate via
  \begin{align*}
    |\nabla^l u(0)|^2 
    &\leq C \Big( \int_{\partial \BB(0, t)} \big\{ |\nabla_x^l \Gamma(y; \lambda)|  + |\nabla_x^l \Phi(y) | \big\} |f(y)| \d\sigma(y)\Big)^2 \\
    \leq  C \Big( \int_{\partial \BB(0,t)} \frac{1}{t^{d - 1 + l}} \Big)^2
    .
  \end{align*}
  Integrating this inequality in $t$ over the interval $(1,2)$ gives \eqref{eq:interiorEstimateDoubleLayer}.
\end{rem}
