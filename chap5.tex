...
We will now derive bounds on the inverse operator of $(1/2) I + \K_\lambda$ from Chapter \ref{chap:3}. 
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\leq C \Big\{  \| (\nabla u)_+ \|_\partial + \Big\|\phi_+ - \frac{1}{r_0^{d - 1}} \int_{\partial\Omega} \phi_+ \d\sigma \Big\|_\partial + \big| \dashint_{\partial\Omega} \phi_+ \d \sigma \big| \Big\}\\
...
\begin{proof}
  ...
  In Chapter \ref{chap:3}, it was already established that the a solution to the Stokes resolvent problem is given by the double layer potentials $u \coloneqq \dlp_\lambda(f)$ and $\phi \coloneqq \dlp_\Phi(f)$.
  ...
  Thus, the central idea of this proof will be to invert the operator $(-1/2)I + \K_\lambda^*$ in order to find the \emph{right} $f$ to plug into the double layer potentials in order ot attain the given boundary data $g \in \Ell^2_n(\partial\Omega)$ as a nontangential limit.
  ...
  \end{align*}
  holds, where for the first equality we applied Theorem \ref{thm:nontangentialLimitDoubleLayer}. The second equality uses the fact that since $(u)^*$ is integrable, Proposition \ref{prop:approximationArgument} and hence the divergence theorem available.
...
\begin{proof}
  Let us denote the trace of $u$ on $\partial\Omega$ by $f$ and let furthermore $w \coloneqq \dlp_\lambda(g)$, with $g \in \Ell^2(\partial\Omega; \C^d)$, be the solution of the $\Ell^2$ Dirichlet problem with boundary data $f \in \Ell^2_n(\partial\Omega)$ as given by Theorem \ref{thm:exAndUniqueSolution}.
  ...
  From the theory of Sobolev spaces it is known that there exists $\varphi_\varepsilon \in \CC^\infty(\overline\Omega; \C^d)$ such that $\big\|\varphi_\varepsilon - u\big\|_{\HH^1(\Omega; \C^d)}^2 \leq ({\varepsilon}/{3})$, see Adams and Fournier \cite[Thm. 3.18]{adams}.
  ...
  \begin{align*}
    ...
    &\leq C\, \big\| u - \varphi_\varepsilon \big\|_{\HH^1(\Omega)}^2 + \int_{\partial\Omega_j} |\varphi_\varepsilon - w|^2 \d \sigma,
  \end{align*}
  where once again $C$ only depends on the Lipschitz character of $\Omega$ and is thus independent of $j$.
  Due to the approximation theorem \ref{thm:smoothApproximation}, we may take $j$ large enough such that also the second summand of the previous inequality is smaller than $\varepsilon/2$.
