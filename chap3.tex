...
\begin{proof}
  The estimate for $(u)^*$ is a direct consequence of Lemma \ref{lem:nontangentialMaximalFunctions} and the estimates on $(\nabla \slp_\lambda(f))^*$ and $(\slp_\Phi(f))^*$:
  ...
  \begin{align*}
    ...
    \leq C \Big\{ \sum_{i = 1}^n \| n_i f \|_{\Ell^p(\partial\Omega; \C^d)} + \sum_{i = 1}^n \| \slp_\Phi(\tilde f^j) \|_{\Ell^p(\partial\Omega)}\Big\} 
  ...
  \begin{align*}
    ...
    = \pv\int_{\partial\Omega} \Big\{ \frac{\partial}{\partial x_i} \big\{ \Gamma_{kj}(x - y; \lambda)\big\} n_i(x) - \Phi_k(x - y) n_j(x) \Big\} g_j(x) \d\sigma(x). 
  \end{align*}
