...
\begin{align}
  ...
  (\slp_{\Phi})_j = \int_{\partial\Omega} \Phi_k(x - y) f_k(y) \d \sigma(y),
\end{align}
...
Another cornerstone in the theory of the single and double layer potentials is the following lemma, see Tolksdorf \cite[Lem. 4.3.2]{tolksdorf}, as will allow us to bring into play the estimates from Section \ref{sec:2.2}.
...
\begin{proof}
  If $\lambda = 0$, the Lemma is known due to Fabes, Kenig and Verchota \cite{fabesKenigVerchota} as a consequence of the seminal result of Coifman et al. \cite{coifman}.
  ...
  \begin{align*}
    \Big| \int_{|y - q| > t} \nabla \Gamma(q - y; \lambda) f(y) \d\sigma(y) \Big|
  ...
  Thus, gluing together \eqref{eq:gtt} and \eqref{eq:applLem31} and letting $N \to \infty$ and noting the geometric series we get the estimate, 
  \begin{align}
    \label{eq:finalgtt}
    \bigg| \int_{|y - q| > t} \nabla \Gamma(q - y; \lambda) f(y) \d\sigma(y) \,\bigg|
  ...
  \begin{align*}
    &\bigg| \int_{t < |q - y| < (2|\lambda|)^{-1/2}} \nabla\Gamma(q - y; \lambda)f(y) \d\sigma(y) \,\bigg| \\
     &\qquad\qquad\leq \int_{t < |q - y| < (2|\lambda|)^{-1/2}} \big|\nabla\Gamma(q - y; \lambda) - \nabla\Gamma(q - y; 0) \big| \, |f(y)| \d\sigma(y) \\
     &\qquad\qquad\quad + \bigg| \int_{t < |q - y| < (2|\lambda|)^{-1/2}} \nabla\Gamma(q - y; 0) f(y)  \d\sigma(y)\, \bigg|.
  \end{align*}
  ...
  \begin{align*}
     &\Big| \int_{t < |q - y| < (2|\lambda|)^{-1/2}} \nabla\Gamma(q - y; 0) f(y)  \d\sigma \Big|
     \leq \Big| \int_{|q - y| > t} \nabla\Gamma(q - y; 0) f(y)  \d\sigma \Big| 
     \leq T_0^*(f)(p).
   \end{align*}
   ...
   \begin{align*}
     &\int_{t < |q - y| < (2|\lambda|)^{-1/2}} |\nabla\Gamma(q - y; \lambda) - \nabla\Gamma(q - y; 0) |f(y)| \d\sigma \\
     &\qquad\qquad\leq C \int_{t < |q - y| < (2|\lambda|)^{-1/2}} |\lambda|^{\frac{1}{2}} |q - y|^{2 - d} |f(y)| \d\sigma,
   \end{align*}
   where $C$ depends on $d$ and $\theta$. 
   Now we choose $N$ such that $2^{N + 1} t > (2|\lambda|)^{-1/2} \geq 2^N t$ holds.
   This leads to the estimate
   \begin{align*}\int_{t < |q - y| < (2|\lambda|)^{-1/2}} |q - y|^{2 - d} |f(y)| \d\sigma
     &\leq \sum_{k = 0}^N \int_{2^kt < |y - P| < 2^{k + 1} t} |q - y|^{2 - d} |f(y)| \d\sigma \\
     &\leq  \sum_{k = 0}^N 2^{k(2 - d)}t^{2 - d}  \int_{\BB(P, 2^{k + 1}t) \cap \partial\Omega} |f(y)| \d\sigma \\
     &\leq  2^d \sum_{k = 0}^N 2^k t \;  \frac{1}{(2^{k + 1}t)^{d - 1}}  \int_{\BB(P, 2^{k + 1}t) \cap \partial\Omega} |f(y)| \d\sigma.
   \end{align*}
   %\begin{align*}
...
%   \end{align*}
   ...
   But $N$ was chosen wisely and thus

   
   %where $C$ again only depends on $d$ and the Lipschitz character of $\Omega$.
   \begin{align*}
     &\qquad\qquad \leq C  |\lambda|^{\frac{1}{2}} t (2^{N} - 1) \,  M_{\partial\Omega}(f)(P) \\
     &\qquad\qquad \leq C |\lambda|^{\frac{1}{2}} (2|\lambda|)^{-\frac{1}{2}} M_{\partial\Omega}(f)(P).
   \end{align*}
...
%  \end{align*}
  ...
  As in Verchota \cite[Lem 1.3]{verchota} the integral may now be bounded by the Hardy-Littlewood maximal operator due to an application of a suitable result from Grafakos \cite[Thm. 2.1.10]{grafakos2014classical} as the kernel $\frac{t}{(t + |y - P|)^d}$ is uniformly integrable and radially decreasing on $\partial\Omega$.
...
%  Taking the supremum over all $x \in \Omega$ the claim follows.
  ...
  \begin{align*}
    &\int_{\partial\Omega} \frac{1}{|P - y|^{d - 2}} \d\sigma(y) \\
    &\qquad\leq \int_{\partial\Omega \cap \BB(P; r_0/4)} \frac{1}{|P - y|^{d - 2}} \d\sigma(y) + \int_{\partial\Omega \setminus \BB(P; r_0/4)} \frac{1}{|P - y|^{d - 2}} \d\sigma(y). \\
    & \qquad\leq C r_0/4 +  \sigma(\partial\Omega) r_0^{2 - d} 4^{d - 2}.
  \end{align*}
...
\intertext{The corresponding pressure $\phi(x) = \dlp_{\phi}(f)(x)$ is defined via}
  \phi(x)
  &= \frac{\partial^2}{\partial x_i \partial x_k} \int_{\partial\Omega} G(y - x; 0) n_i(y) f_k(y) \d\sigma(y) \nonumber\\
...
The next theorem will give us a suitable operator which maps a given function $f \in \Ell^p(\partial\Omega; \C^d)$ to boundary values of $u = \dlp_\lambda(f)$ in the form of nontangential limits.
