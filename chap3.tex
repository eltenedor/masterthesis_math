...
We note that due to the new two dimensional estimates on fundamental solutions in Chapter \ref{chap:2}, namely the continuation of Theorem \ref{thm:differenceFundamentalSolutionStokes} and Theorem \ref{thm:fundamentalMatrixEstimate} for the case $d=2$, all results from Chapter 3 that are relevant to the analysis of the $\Ell^2$ Dirichlet problem associated to the Stokes resolvent problem of Shen's seminal paper \cite{Shen2012} could be  extended in a straightforward way.
...
\begin{proof}
  If $\lambda = 0$, the Lemma is known \cite{fabesKenigVerchota} as a consequence of the seminal result of Coifman et al. \cite{coifman}.
...
\begin{proof}
  ...
  As a consequence, $\K$ is compact since the limit of compact oprators with respect to the operator norm gives again a compact operator.
\end{proof}
