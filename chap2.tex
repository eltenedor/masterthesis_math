...
\begin{lem}
  ...
  If $w \in \CC^{k}(\Omega) \cap \CC^0(\overline\Omega)$ is a solution to $\Delta w = f$ in $\BB(x,r)$. Then
  \begin{align}
...
\begin{proof}
  If $l = 1$, estimate \eqref{eq:interiorEstimatePoisson} is a consequence of the \emph{comparison principle} and a proof of this fact can be found in the book of Gilbarg and Trudinger \cite[Sec. 3.4, Eq. (3.16)]{gilbarg}.
  We can now use this estimate to inductively deduce the estimates for higher derivatives.
  Note that by translating and rescaling like
  \begin{align*}
...
%    \end{align*}
  ...
  \item For the last case $d = 2$ and $l = 3$, we will directly calculate the estimate using the asymptotic expansion of $H_0^{(1)}(z)$ with $z = k|x|$. The calculations are omitted from this chapter. Instead, they can be found in the appendix chapter of this thesis. \qedhere
%    \begin{align*}
%      H_0^{(1)}(z) 
%      &= J_0(z) + \ii Y_0(z) \\
...
\end{thm}
 ...
 %One important difference compared to $d \geq 3$ is that it is not possible to derive an estimate of the form $|\nabla_x^l \Gamma(x; \lambda)| \leq C |x|^{-l}$.
  ...
  One important technique in the following chapter will be to reduce problems formulated for $\Gamma(x; \lambda)$ to problems formulated in $\Gamma(x; 0)$ plus the difference $\Gamma(x; \lambda) - \Gamma(x; 0)$.
