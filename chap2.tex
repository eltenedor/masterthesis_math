...
\end{align}
Furthermore we will denote The leading coefficient of the asymptotic expansions of the Hankel functions \eqref{eq:asymptoticd4}-\eqref{eq:asymptoticd7} for $d \geq 3$ will be denoted as
\begin{align}
  \label{eq:Defnad}
...
\begin{proof}
  \begin{enumerate}[(a)]
    \item In this part we will show that the desired estimate \eqref{eq:HelmholtzLaplaceDifferenceEstimate} holds if we assume that $|\lambda| |x|^2 > \frac{1}{2}$.
...
\end{thm}
  ...
  \begin{rem}
    It is important to note that the proof would also work for $d = 2$ and $l=1$ yielding an estimate of the form
    \begin{align*}
    \end{align*}
    which offers a different behavior as estimates of the form \ref{}.
    It will be important in later chapters to have an estimate on $\nabla^l_x \Gamma(x; \lambda)$ that does not depend on $\lambda$.
  \end{rem}
...
\begin{proof}
  ...
  We start by considering the cases $d = 3$ and $d \geq 5$.
      Taking into account identity \eqref{eq:fundamentalSolutionStokes35}, we have for all $\alpha, \beta = 1,\dots,d$
      \begin{align*}
        &\Gamma_{\alpha\beta}(x; \lambda) - \Gamma_{\alpha\beta}(x; 0) \\
        &\qquad\qquad= \big\{ G(x; \lambda) - G(x; 0) \big\}\delta_{\alpha\beta}  \\
        &\qquad\qquad\qquad- \frac{1}{\lambda} \frac{\partial^2}{\partial x_\alpha \partial x_\beta} \bigg\{ G(x; \lambda) - G(x; 0) + \frac{\lambda}{2 \omega_d (d - 4) (d - 2) |x|^{d - 4}} \bigg\}.
      \end{align*}
...
\begin{align*}
 ...
 &= -\frac{2\ii (2\pi)^{\frac{d}{2} - 1}}{\omega_d (d - 4) (d - 2)}.
  \end{align*}
    Using Relation and the functional equation of the Gamma function twice, we see that
    \begin{align*}
...
similar to \eqref{eq:estimateDerivativesd7} the estimate
\begin{align}
...
\begin{align*}
  ...
  &\hspace{3cm}= \frac{\partial^2}{\partial x_\alpha x_\beta} \Big\{ \frac{\ii}{4(2\pi)^{\frac{3}{2}}} \cdot \frac{1}{|x|^3} \Big[ z^{\frac{3}{2}} H_{\frac{3}{2}}^{(1)}(z) - a_5 - b_5 z^2 - w z^3 \Big] \Big\},
\end{align*}
...
In the case $d = 4$ we use \eqref{eq:fundamentalSolutionStokes4} to reformulate the difference
\begin{align*}
  &\Gamma_{\alpha\beta}(x; \lambda) - \Gamma_{\alpha\beta}(x; 0) \\
  ...
  &\quad= \big\{ G(x; \lambda) - G(x; 0) \big\} \delta_{\alpha\beta} - \frac{\ii}{\lambda} \frac{\partial^2}{\partial x_\alpha \partial x_\beta} \Big\{ \frac{1}{8 \pi |x|^2} \Big[ z H_1^{(1)}(z) - a_4 - w z^2 - b_4 z^2 \log(z)\,\Big] \Big\},
\end{align*}
...
For $d = 2$ the estimate follows from a direct calculation which is carried out in the appendix of this thesis.
\end{proof}

We can now use the assumption $|\lambda||x|^2 \leq ({1}/{2})$  to unify the structure of the estimates for $d \geq 2$.
\begin{cor}
