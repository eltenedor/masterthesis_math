...
\begin{proof}
  For $|\lambda| = 1$ we have $\Re(\lambda) = \cos(\varphi)$ and $\Im(\lambda) = \sin(\varphi)$ for some $\varphi \in (0,\varphi - \theta)$.
  Set
  \begin{align*}
    \alpha = \frac{1 - \cos(\varphi - \theta)}{\sin(\pi - \theta)} \geq \frac{1 - \cos(\varphi)}{\sin(\varphi)}.
  \end{align*}
  Then we have
  \begin{align*}
